 ...
 * @see https://schema.ocsf.io/1.6.0/classes/iam
 */
...
export const Iam = {
  parse: (data: unknown): IamType => IamSchema.parse(preprocess(data)),

  safeParse: (data: unknown): { success: true; data: IamType } | { success: false; error: z.ZodError } => {
    try {
      const preprocessed = preprocess(data);
      return IamSchema.safeParse(preprocessed);
    } catch (error) {
