...
import type { DceRpcType } from '../objects/dce_rpc.js';
...
import { DceRpc } from '../objects/dce_rpc.js';
...
const SmbActivitySchema: z.ZodType<SmbActivityType> = z.strictObject({
  /** The normalized identifier of the activity that triggered the event. */
...
export const SmbActivity = {
  parse: (data: unknown): SmbActivityType => SmbActivitySchema.parse(preprocess(data)),

  safeParse: (data: unknown): { success: true; data: SmbActivityType } | { success: false; error: z.ZodError } => {
    try {
      const preprocessed = preprocess(data);
      return SmbActivitySchema.safeParse(preprocessed);
    } catch (error) {
