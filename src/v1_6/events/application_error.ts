...
const ApplicationErrorSchema: z.ZodType<ApplicationErrorType> = z.strictObject({
  /** The normalized identifier of the activity that triggered the event. */
...
export const ApplicationError = {
  parse: (data: unknown): ApplicationErrorType => ApplicationErrorSchema.parse(preprocess(data)),

  safeParse: (data: unknown): { success: true; data: ApplicationErrorType } | { success: false; error: z.ZodError } => {
    try {
      const preprocessed = preprocess(data);
      return ApplicationErrorSchema.safeParse(preprocessed);
    } catch (error) {
