...
const FtpActivitySchema: z.ZodType<FtpActivityType> = z.strictObject({
  /** The normalized identifier of the activity that triggered the event. */
...
export const FtpActivity = {
  parse: (data: unknown): FtpActivityType => FtpActivitySchema.parse(preprocess(data)),

  safeParse: (data: unknown): { success: true; data: FtpActivityType } | { success: false; error: z.ZodError } => {
    try {
      const preprocessed = preprocess(data);
      return FtpActivitySchema.safeParse(preprocessed);
    } catch (error) {
