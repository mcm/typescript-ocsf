 ...
 */
export interface NetworkRemediationActivityType {
...
const NetworkRemediationActivitySchema: z.ZodType<NetworkRemediationActivityType> = z.strictObject({
  /** Matches the MITRE D3FEND™ Tactic. Note: the Model and Detect Tactics are not supported as remediations by the OCSF Remediation event class. */
...
export const NetworkRemediationActivity = {
  parse: (data: unknown): NetworkRemediationActivityType => NetworkRemediationActivitySchema.parse(preprocess(data)),

  safeParse: (data: unknown): { success: true; data: NetworkRemediationActivityType } | { success: false; error: z.ZodError } => {
    try {
      const preprocessed = preprocess(data);
      return NetworkRemediationActivitySchema.safeParse(preprocessed);
    } catch (error) {
