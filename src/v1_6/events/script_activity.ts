 ...
 */
export interface ScriptActivityType {
...
import { Script } from '../objects/script.js';
...
export const ScriptActivity = {
  parse: (data: unknown): ScriptActivityType => ScriptActivitySchema.parse(preprocess(data)),

  safeParse: (data: unknown): { success: true; data: ScriptActivityType } | { success: false; error: z.ZodError } => {
    try {
      const preprocessed = preprocess(data);
      return ScriptActivitySchema.safeParse(preprocessed);
    } catch (error) {
