...
const ModuleActivitySchema: z.ZodType<ModuleActivityType> = z.strictObject({
  /** The normalized identifier of the activity that triggered the event. */
...
export const ModuleActivity = {
  parse: (data: unknown): ModuleActivityType => ModuleActivitySchema.parse(preprocess(data)),

  safeParse: (data: unknown): { success: true; data: ModuleActivityType } | { success: false; error: z.ZodError } => {
    try {
      const preprocessed = preprocess(data);
      return ModuleActivitySchema.safeParse(preprocessed);
    } catch (error) {
