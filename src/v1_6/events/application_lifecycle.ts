 ...
 */
export interface ApplicationLifecycleType {
...
const ApplicationLifecycleSchema: z.ZodType<ApplicationLifecycleType> = z.strictObject({
  /** The normalized identifier of the activity that triggered the event. */
  activity_id: z.union([z.literal(0), z.literal(1), z.literal(2), z.literal(3), z.literal(4), z.literal(5), z.literal(6), z.literal(7), z.literal(8), z.literal(99)]),
  /** The event activity name, as defined by the activity_id. */
...
export const ApplicationLifecycle = {
  parse: (data: unknown): ApplicationLifecycleType => ApplicationLifecycleSchema.parse(preprocess(data)),

  safeParse: (data: unknown): { success: true; data: ApplicationLifecycleType } | { success: false; error: z.ZodError } => {
    try {
      const preprocessed = preprocess(data);
      return ApplicationLifecycleSchema.safeParse(preprocessed);
    } catch (error) {
