 ...
 * OCSF Class UID: 5000
 * Category: Discovery
 * @see https://schema.ocsf.io/1.6.0/classes/discovery
 */
...
export const Discovery = {
  parse: (data: unknown): DiscoveryType => DiscoverySchema.parse(preprocess(data)),

  safeParse: (data: unknown): { success: true; data: DiscoveryType } | { success: false; error: z.ZodError } => {
    try {
      const preprocessed = preprocess(data);
      return DiscoverySchema.safeParse(preprocessed);
    } catch (error) {
