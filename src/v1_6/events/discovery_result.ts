 ...
 * @see https://schema.ocsf.io/1.6.0/classes/discovery_result
 */
...
const DiscoveryResultSchema: z.ZodType<DiscoveryResultType> = z.strictObject({
  /** The normalized identifier of the activity that triggered the event. */
...
export const DiscoveryResult = {
  parse: (data: unknown): DiscoveryResultType => DiscoveryResultSchema.parse(preprocess(data)),

  safeParse: (data: unknown): { success: true; data: DiscoveryResultType } | { success: false; error: z.ZodError } => {
    try {
      const preprocessed = preprocess(data);
      return DiscoveryResultSchema.safeParse(preprocessed);
    } catch (error) {
