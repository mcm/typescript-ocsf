...
import type { AssessmentType } from '../objects/assessment.js';
...
export interface ConfigStateType {
  ...
  /** The CIS Benchmark Result object captures results generated from benchmark evaluations as defined by the Center for Internet Security (CIS). */
  cis_benchmark_result?: CisBenchmarkResultType;
...
import { Assessment } from '../objects/assessment.js';
import { CisBenchmarkResult } from '../objects/cis_benchmark_result.js';
...
const ConfigStateSchema: z.ZodType<ConfigStateType> = z.strictObject({
  /** The normalized identifier of the activity that triggered the event. */
...
export const ConfigState = {
  parse: (data: unknown): ConfigStateType => ConfigStateSchema.parse(preprocess(data)),

  safeParse: (data: unknown): { success: true; data: ConfigStateType } | { success: false; error: z.ZodError } => {
    try {
      const preprocessed = preprocess(data);
      return ConfigStateSchema.safeParse(preprocessed);
    } catch (error) {
