 ...
 * @see https://schema.ocsf.io/1.6.0/classes/system
 */
...
const SystemSchema: z.ZodType<SystemType> = z.strictObject({
  /** The normalized identifier of the activity that triggered the event. */
...
export const System = {
  parse: (data: unknown): SystemType => SystemSchema.parse(preprocess(data)),

  safeParse: (data: unknown): { success: true; data: SystemType } | { success: false; error: z.ZodError } => {
    try {
      const preprocessed = preprocess(data);
      return SystemSchema.safeParse(preprocessed);
    } catch (error) {
