...
const UserAccessSchema: z.ZodType<UserAccessType> = z.strictObject({
  /** The normalized identifier of the activity that triggered the event. */
...
export const UserAccess = {
  parse: (data: unknown): UserAccessType => UserAccessSchema.parse(preprocess(data)),

  safeParse: (data: unknown): { success: true; data: UserAccessType } | { success: false; error: z.ZodError } => {
    try {
      const preprocessed = preprocess(data);
      return UserAccessSchema.safeParse(preprocessed);
    } catch (error) {
