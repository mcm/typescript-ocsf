 ...
 * OCSF Class UID: 4000
 * Category: Network
 * @see https://schema.ocsf.io/1.5.0/classes/network
 */
...
const NetworkSchema: z.ZodType<NetworkType> = z.strictObject({
  /** The normalized identifier of the activity that triggered the event. */
...
export const Network = {
  parse: (data: unknown): NetworkType => NetworkSchema.parse(preprocess(data)),

  safeParse: (data: unknown): { success: true; data: NetworkType } | { success: false; error: z.ZodError } => {
    try {
      const preprocessed = preprocess(data);
      return NetworkSchema.safeParse(preprocessed);
    } catch (error) {
