...
const JobQuerySchema: z.ZodType<JobQueryType> = z.strictObject({
  /** The normalized identifier of the activity that triggered the event. */
...
export const JobQuery = {
  parse: (data: unknown): JobQueryType => JobQuerySchema.parse(preprocess(data)),

  safeParse: (data: unknown): { success: true; data: JobQueryType } | { success: false; error: z.ZodError } => {
    try {
      const preprocessed = preprocess(data);
      return JobQuerySchema.safeParse(preprocessed);
    } catch (error) {
