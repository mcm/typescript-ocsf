...
const EmailUrlActivitySchema: z.ZodType<EmailUrlActivityType> = z.strictObject({
  /** The normalized identifier of the activity that triggered the event. */
...
export const EmailUrlActivity = {
  parse: (data: unknown): EmailUrlActivityType => EmailUrlActivitySchema.parse(preprocess(data)),

  safeParse: (data: unknown): { success: true; data: EmailUrlActivityType } | { success: false; error: z.ZodError } => {
    try {
      const preprocessed = preprocess(data);
      return EmailUrlActivitySchema.safeParse(preprocessed);
    } catch (error) {
