 ...
 *
 * OCSF Class UID: 5001
 * Category: Device Inventory Info
 * @see https://schema.ocsf.io/1.5.0/classes/inventory_info
 */
export interface InventoryInfoType {
...
const InventoryInfoSchema: z.ZodType<InventoryInfoType> = z.strictObject({
  /** The normalized identifier of the activity that triggered the event. */
...
export const InventoryInfo = {
  parse: (data: unknown): InventoryInfoType => InventoryInfoSchema.parse(preprocess(data)),

  safeParse: (data: unknown): { success: true; data: InventoryInfoType } | { success: false; error: z.ZodError } => {
    try {
      const preprocessed = preprocess(data);
      return InventoryInfoSchema.safeParse(preprocessed);
    } catch (error) {
