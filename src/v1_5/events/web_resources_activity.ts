 ...
 */
export interface WebResourcesActivityType {
  ...
  web_resources_result?: WebResourceType[];
}
...
const WebResourcesActivitySchema: z.ZodType<WebResourcesActivityType> = z.strictObject({
  /** The normalized identifier of the activity that triggered the event. */
...
export const WebResourcesActivity = {
  parse: (data: unknown): WebResourcesActivityType => WebResourcesActivitySchema.parse(preprocess(data)),

  safeParse: (data: unknown): { success: true; data: WebResourcesActivityType } | { success: false; error: z.ZodError } => {
    try {
      const preprocessed = preprocess(data);
      return WebResourcesActivitySchema.safeParse(preprocessed);
    } catch (error) {
