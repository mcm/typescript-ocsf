 ...
 */
export interface KernelExtensionActivityType {
...
import { KernelDriver } from '../objects/kernel_driver.js';
...
const KernelExtensionActivitySchema: z.ZodType<KernelExtensionActivityType> = z.strictObject({
  /** The normalized identifier of the activity that triggered the event. */
...
export const KernelExtensionActivity = {
  parse: (data: unknown): KernelExtensionActivityType => KernelExtensionActivitySchema.parse(preprocess(data)),

  safeParse: (data: unknown): { success: true; data: KernelExtensionActivityType } | { success: false; error: z.ZodError } => {
    try {
      const preprocessed = preprocess(data);
      return KernelExtensionActivitySchema.safeParse(preprocessed);
    } catch (error) {
