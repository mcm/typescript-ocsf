 ...
 */
export interface ModuleQueryType {
...
const ModuleQuerySchema: z.ZodType<ModuleQueryType> = z.strictObject({
  /** The normalized identifier of the activity that triggered the event. */
...
export const ModuleQuery = {
  parse: (data: unknown): ModuleQueryType => ModuleQuerySchema.parse(preprocess(data)),

  safeParse: (data: unknown): { success: true; data: ModuleQueryType } | { success: false; error: z.ZodError } => {
    try {
      const preprocessed = preprocess(data);
      return ModuleQuerySchema.safeParse(preprocessed);
    } catch (error) {
