...
import { ProcessRemediationActivityActivityId } from '../enums/process_remediation_activity_activity_id.js';
 ...
 *
 * OCSF Class UID: 7003
 * Category: Process Remediation Activity
 * @see https://schema.ocsf.io/1.5.0/classes/process_remediation_activity
 */
export interface ProcessRemediationActivityType {
...
const ProcessRemediationActivitySchema = z.strictObject({
  /** Matches the MITRE D3FEND™ Tactic. Note: the Model and Detect Tactics are not supported as remediations by the OCSF Remediation event class. */
...
export const ProcessRemediationActivity = {
  parse: (data: unknown): ProcessRemediationActivityType => ProcessRemediationActivitySchema.parse(preprocess(data)),

  safeParse: (data: unknown): { success: true; data: ProcessRemediationActivityType } | { success: false; error: z.ZodError } => {
    try {
      const preprocessed = preprocess(data);
      return ProcessRemediationActivitySchema.safeParse(preprocessed);
    } catch (error) {
  ...
  schema: ProcessRemediationActivitySchema,

  // Static enum references for convenience
  ActivityId: ProcessRemediationActivityActivityId,
