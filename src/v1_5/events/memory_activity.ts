 ...
 */
export interface MemoryActivityType {
...
const MemoryActivitySchema: z.ZodType<MemoryActivityType> = z.strictObject({
  /** The normalized identifier of the activity that triggered the event. */
...
export const MemoryActivity = {
  parse: (data: unknown): MemoryActivityType => MemoryActivitySchema.parse(preprocess(data)),

  safeParse: (data: unknown): { success: true; data: MemoryActivityType } | { success: false; error: z.ZodError } => {
    try {
      const preprocessed = preprocess(data);
      return MemoryActivitySchema.safeParse(preprocessed);
    } catch (error) {
