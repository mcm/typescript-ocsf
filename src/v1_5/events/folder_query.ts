 ...
 */
export interface FolderQueryType {
...
const FolderQuerySchema: z.ZodType<FolderQueryType> = z.strictObject({
  /** The normalized identifier of the activity that triggered the event. */
...
export const FolderQuery = {
  parse: (data: unknown): FolderQueryType => FolderQuerySchema.parse(preprocess(data)),

  safeParse: (data: unknown): { success: true; data: FolderQueryType } | { success: false; error: z.ZodError } => {
    try {
      const preprocessed = preprocess(data);
      return FolderQuerySchema.safeParse(preprocessed);
    } catch (error) {
