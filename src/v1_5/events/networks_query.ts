 ...
 */
export interface NetworksQueryType {
...
const NetworksQuerySchema: z.ZodType<NetworksQueryType> = z.strictObject({
  /** The normalized identifier of the activity that triggered the event. */
...
export const NetworksQuery = {
  parse: (data: unknown): NetworksQueryType => NetworksQuerySchema.parse(preprocess(data)),

  safeParse: (data: unknown): { success: true; data: NetworksQueryType } | { success: false; error: z.ZodError } => {
    try {
      const preprocessed = preprocess(data);
      return NetworksQuerySchema.safeParse(preprocessed);
    } catch (error) {
