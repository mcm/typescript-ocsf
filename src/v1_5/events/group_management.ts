 ...
 */
export interface GroupManagementType {
...
const GroupManagementSchema: z.ZodType<GroupManagementType> = z.strictObject({
  /** The normalized identifier of the activity that triggered the event. */
...
export const GroupManagement = {
  parse: (data: unknown): GroupManagementType => GroupManagementSchema.parse(preprocess(data)),

  safeParse: (data: unknown): { success: true; data: GroupManagementType } | { success: false; error: z.ZodError } => {
    try {
      const preprocessed = preprocess(data);
      return GroupManagementSchema.safeParse(preprocessed);
    } catch (error) {
