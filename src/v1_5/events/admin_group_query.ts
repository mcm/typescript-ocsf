 ...
 */
export interface AdminGroupQueryType {
...
const AdminGroupQuerySchema: z.ZodType<AdminGroupQueryType> = z.strictObject({
  /** The normalized identifier of the activity that triggered the event. */
...
export const AdminGroupQuery = {
  parse: (data: unknown): AdminGroupQueryType => AdminGroupQuerySchema.parse(preprocess(data)),

  safeParse: (data: unknown): { success: true; data: AdminGroupQueryType } | { success: false; error: z.ZodError } => {
    try {
      const preprocessed = preprocess(data);
      return AdminGroupQuerySchema.safeParse(preprocessed);
    } catch (error) {
