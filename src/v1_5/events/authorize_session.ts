 ...
 */
export interface AuthorizeSessionType {
...
const AuthorizeSessionSchema: z.ZodType<AuthorizeSessionType> = z.strictObject({
  /** The normalized identifier of the activity that triggered the event. */
...
export const AuthorizeSession = {
  parse: (data: unknown): AuthorizeSessionType => AuthorizeSessionSchema.parse(preprocess(data)),

  safeParse: (data: unknown): { success: true; data: AuthorizeSessionType } | { success: false; error: z.ZodError } => {
    try {
      const preprocessed = preprocess(data);
      return AuthorizeSessionSchema.safeParse(preprocessed);
    } catch (error) {
