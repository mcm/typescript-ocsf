 ...
 */
export interface RemediationActivityType {
...
const RemediationActivitySchema: z.ZodType<RemediationActivityType> = z.strictObject({
  /** Matches the MITRE D3FEND™ Tactic. Note: the Model and Detect Tactics are not supported as remediations by the OCSF Remediation event class. */
...
export const RemediationActivity = {
  parse: (data: unknown): RemediationActivityType => RemediationActivitySchema.parse(preprocess(data)),

  safeParse: (data: unknown): { success: true; data: RemediationActivityType } | { success: false; error: z.ZodError } => {
    try {
      const preprocessed = preprocess(data);
      return RemediationActivitySchema.safeParse(preprocessed);
    } catch (error) {
