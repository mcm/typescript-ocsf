 ...
 */
export interface FileQueryType {
...
const FileQuerySchema: z.ZodType<FileQueryType> = z.strictObject({
  /** The normalized identifier of the activity that triggered the event. */
...
export const FileQuery = {
  parse: (data: unknown): FileQueryType => FileQuerySchema.parse(preprocess(data)),

  safeParse: (data: unknown): { success: true; data: FileQueryType } | { success: false; error: z.ZodError } => {
    try {
      const preprocessed = preprocess(data);
      return FileQuerySchema.safeParse(preprocessed);
    } catch (error) {
