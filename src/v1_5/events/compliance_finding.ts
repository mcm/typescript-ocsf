...
export const ComplianceFinding = {
  parse: (data: unknown): ComplianceFindingType => ComplianceFindingSchema.parse(preprocess(data)),

  safeParse: (data: unknown): { success: true; data: ComplianceFindingType } | { success: false; error: z.ZodError } => {
    try {
      const preprocessed = preprocess(data);
      return ComplianceFindingSchema.safeParse(preprocessed);
    } catch (error) {
