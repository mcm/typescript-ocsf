 ...
 */
export interface OsintInventoryInfoType {
...
const OsintInventoryInfoSchema: z.ZodType<OsintInventoryInfoType> = z.strictObject({
  /** The normalized identifier of the activity that triggered the event. */
...
export const OsintInventoryInfo = {
  parse: (data: unknown): OsintInventoryInfoType => OsintInventoryInfoSchema.parse(preprocess(data)),

  safeParse: (data: unknown): { success: true; data: OsintInventoryInfoType } | { success: false; error: z.ZodError } => {
    try {
      const preprocessed = preprocess(data);
      return OsintInventoryInfoSchema.safeParse(preprocessed);
    } catch (error) {
