 ...
 */
export interface PeripheralDeviceQueryType {
...
export const PeripheralDeviceQuery = {
  parse: (data: unknown): PeripheralDeviceQueryType => PeripheralDeviceQuerySchema.parse(preprocess(data)),

  safeParse: (data: unknown): { success: true; data: PeripheralDeviceQueryType } | { success: false; error: z.ZodError } => {
    try {
      const preprocessed = preprocess(data);
      return PeripheralDeviceQuerySchema.safeParse(preprocessed);
    } catch (error) {
