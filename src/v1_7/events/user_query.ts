...
import { UserQueryActivityId } from '../enums/user_query_activity_id.js';
...
const UserQuerySchema = z.strictObject({
  /** The normalized identifier of the activity that triggered the event. */
...
export const UserQuery = {
  parse: (data: unknown): UserQueryType => UserQuerySchema.parse(preprocess(data)),

  safeParse: (data: unknown): { success: true; data: UserQueryType } | { success: false; error: z.ZodError } => {
    try {
      const preprocessed = preprocess(data);
      return UserQuerySchema.safeParse(preprocessed);
    } catch (error) {
  ...
  ActivityId: UserQueryActivityId,
