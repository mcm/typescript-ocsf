 ...
 */
export interface KernelObjectQueryType {
...
const KernelObjectQuerySchema: z.ZodType<KernelObjectQueryType> = z.strictObject({
  /** The normalized identifier of the activity that triggered the event. */
...
export const KernelObjectQuery = {
  parse: (data: unknown): KernelObjectQueryType => KernelObjectQuerySchema.parse(preprocess(data)),

  safeParse: (data: unknown): { success: true; data: KernelObjectQueryType } | { success: false; error: z.ZodError } => {
    try {
      const preprocessed = preprocess(data);
      return KernelObjectQuerySchema.safeParse(preprocessed);
    } catch (error) {
