 ...
 * @see https://schema.ocsf.io/1.7.0/classes/unmanned_systems
 */
export interface UnmannedSystemsType {
...
const UnmannedSystemsSchema: z.ZodType<UnmannedSystemsType> = z.strictObject({
  /** The normalized identifier of the activity that triggered the event. */
...
export const UnmannedSystems = {
  parse: (data: unknown): UnmannedSystemsType => UnmannedSystemsSchema.parse(preprocess(data)),

  safeParse: (data: unknown): { success: true; data: UnmannedSystemsType } | { success: false; error: z.ZodError } => {
    try {
      const preprocessed = preprocess(data);
      return UnmannedSystemsSchema.safeParse(preprocessed);
    } catch (error) {
