...
import type { ApiType } from '../objects/api.js';
...
const ApiActivitySchema: z.ZodType<ApiActivityType> = z.strictObject({
  /** The normalized identifier of the activity that triggered the event. */
...
export const ApiActivity = {
  parse: (data: unknown): ApiActivityType => ApiActivitySchema.parse(preprocess(data)),

  safeParse: (data: unknown): { success: true; data: ApiActivityType } | { success: false; error: z.ZodError } => {
    try {
      const preprocessed = preprocess(data);
      return ApiActivitySchema.safeParse(preprocessed);
    } catch (error) {
