...
import type { CloudType } from '../objects/cloud.js';
import type { ContainerType } from '../objects/container.js';
import type { DatabaseType } from '../objects/database.js';
import type { DatabucketType } from '../objects/databucket.js';
import type { IdpType } from '../objects/idp.js';
 ...
 */
export interface CloudResourcesInventoryInfoType {
...
import { Idp } from '../objects/idp.js';
...
export const CloudResourcesInventoryInfo = {
  parse: (data: unknown): CloudResourcesInventoryInfoType => CloudResourcesInventoryInfoSchema.parse(preprocess(data)),

  safeParse: (data: unknown): { success: true; data: CloudResourcesInventoryInfoType } | { success: false; error: z.ZodError } => {
    try {
      const preprocessed = preprocess(data);
      return CloudResourcesInventoryInfoSchema.safeParse(preprocessed);
    } catch (error) {
