 ...
 */
export interface StartupItemQueryType {
...
import { StartupItem } from '../objects/startup_item.js';
...
const StartupItemQuerySchema: z.ZodType<StartupItemQueryType> = z.strictObject({
  /** The normalized identifier of the activity that triggered the event. */
...
export const StartupItemQuery = {
  parse: (data: unknown): StartupItemQueryType => StartupItemQuerySchema.parse(preprocess(data)),

  safeParse: (data: unknown): { success: true; data: StartupItemQueryType } | { success: false; error: z.ZodError } => {
    try {
      const preprocessed = preprocess(data);
      return StartupItemQuerySchema.safeParse(preprocessed);
    } catch (error) {
