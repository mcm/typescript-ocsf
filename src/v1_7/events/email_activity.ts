...
const EmailActivitySchema: z.ZodType<EmailActivityType> = z.strictObject({
  /** The normalized identifier of the activity that triggered the event. */
  activity_id: z.union([z.literal(0), z.literal(1), z.literal(2), z.literal(3), z.literal(4), z.literal(5), z.literal(99)]),
  /** The event activity name, as defined by the activity_id. */
...
export const EmailActivity = {
  parse: (data: unknown): EmailActivityType => EmailActivitySchema.parse(preprocess(data)),

  safeParse: (data: unknown): { success: true; data: EmailActivityType } | { success: false; error: z.ZodError } => {
    try {
      const preprocessed = preprocess(data);
      return EmailActivitySchema.safeParse(preprocessed);
    } catch (error) {
