...
import { LaunchTypeId } from '../enums/index.js';
...
const LAUNCH_TYPE_ID_LABELS: Record<number, string> = {
  0: "Unknown",
  1: "Spawn",
  2: "Fork",
  3: "Exec",
...
const SIBLING_PAIRS: readonly SiblingPair[] = [
  ...
  { idField: "launch_type_id", labelField: "launch_type", labels: LAUNCH_TYPE_ID_LABELS },
];
...
const ProcessActivitySchema = z.strictObject({
  ...
  /** The normalized identifier of the process injection method. */
  injection_type_id: z.union([z.literal(0), z.literal(1), z.literal(2), z.literal(3), z.literal(99)]).optional(),
  /** The specific type of Launch activity, normalized to the caption of the launch_type_id value. In the case of Other it is defined by the event source. */
  launch_type: z.string().optional(),
  /** The normalized identifier for the specific type of Launch activity. */
  launch_type_id: z.union([z.literal(0), z.literal(1), z.literal(2), z.literal(3), z.literal(99)]).optional(),
  /** The module that was injected by the actor process. */
...
export const ProcessActivity = {
  parse: (data: unknown): ProcessActivityType => ProcessActivitySchema.parse(preprocess(data)),

  safeParse: (data: unknown): { success: true; data: ProcessActivityType } | { success: false; error: z.ZodError } => {
    try {
      const preprocessed = preprocess(data);
      return ProcessActivitySchema.safeParse(preprocessed);
    } catch (error) {
