...
export const SessionQuery = {
  parse: (data: unknown): SessionQueryType => SessionQuerySchema.parse(preprocess(data)),

  safeParse: (data: unknown): { success: true; data: SessionQueryType } | { success: false; error: z.ZodError } => {
    try {
      const preprocessed = preprocess(data);
      return SessionQuerySchema.safeParse(preprocessed);
    } catch (error) {
