 ...
 */
export interface PatchStateType {
...
import { KbArticle } from '../objects/kb_article.js';
...
const PatchStateSchema: z.ZodType<PatchStateType> = z.strictObject({
  /** The normalized identifier of the activity that triggered the event. */
...
export const PatchState = {
  parse: (data: unknown): PatchStateType => PatchStateSchema.parse(preprocess(data)),

  safeParse: (data: unknown): { success: true; data: PatchStateType } | { success: false; error: z.ZodError } => {
    try {
      const preprocessed = preprocess(data);
      return PatchStateSchema.safeParse(preprocessed);
    } catch (error) {
