 ...
 */
export interface ScheduledJobActivityType {
...
const ScheduledJobActivitySchema: z.ZodType<ScheduledJobActivityType> = z.strictObject({
  /** The normalized identifier of the activity that triggered the event. */
...
export const ScheduledJobActivity = {
  parse: (data: unknown): ScheduledJobActivityType => ScheduledJobActivitySchema.parse(preprocess(data)),

  safeParse: (data: unknown): { success: true; data: ScheduledJobActivityType } | { success: false; error: z.ZodError } => {
    try {
      const preprocessed = preprocess(data);
      return ScheduledJobActivitySchema.safeParse(preprocessed);
    } catch (error) {
