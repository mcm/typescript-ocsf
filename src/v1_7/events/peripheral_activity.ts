 ...
 */
export interface PeripheralActivityType {
...
import { PeripheralDevice } from '../objects/peripheral_device.js';
...
const PeripheralActivitySchema: z.ZodType<PeripheralActivityType> = z.strictObject({
  /** The normalized identifier of the activity that triggered the event. */
...
export const PeripheralActivity = {
  parse: (data: unknown): PeripheralActivityType => PeripheralActivitySchema.parse(preprocess(data)),

  safeParse: (data: unknown): { success: true; data: PeripheralActivityType } | { success: false; error: z.ZodError } => {
    try {
      const preprocessed = preprocess(data);
      return PeripheralActivitySchema.safeParse(preprocessed);
    } catch (error) {
