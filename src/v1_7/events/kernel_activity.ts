 ...
 */
export interface KernelActivityType {
...
import { Kernel } from '../objects/kernel.js';
...
const KernelActivitySchema: z.ZodType<KernelActivityType> = z.strictObject({
  /** The normalized identifier of the activity that triggered the event. */
...
export const KernelActivity = {
  parse: (data: unknown): KernelActivityType => KernelActivitySchema.parse(preprocess(data)),

  safeParse: (data: unknown): { success: true; data: KernelActivityType } | { success: false; error: z.ZodError } => {
    try {
      const preprocessed = preprocess(data);
      return KernelActivitySchema.safeParse(preprocessed);
    } catch (error) {
