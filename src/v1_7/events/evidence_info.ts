 ...
 */
export interface EvidenceInfoType {
  ...
  query_evidence: QueryEvidenceType;
}
...
import { QueryEvidence } from '../objects/query_evidence.js';
...
const EvidenceInfoSchema: z.ZodType<EvidenceInfoType> = z.strictObject({
  /** The normalized identifier of the activity that triggered the event. */
...
export const EvidenceInfo = {
  parse: (data: unknown): EvidenceInfoType => EvidenceInfoSchema.parse(preprocess(data)),

  safeParse: (data: unknown): { success: true; data: EvidenceInfoType } | { success: false; error: z.ZodError } => {
    try {
      const preprocessed = preprocess(data);
      return EvidenceInfoSchema.safeParse(preprocessed);
    } catch (error) {
