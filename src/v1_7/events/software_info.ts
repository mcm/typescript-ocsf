...
import type { PackageType } from '../objects/package.js';
 ...
 */
export interface SoftwareInfoType {
...
import { Sbom } from '../objects/sbom.js';
...
const SoftwareInfoSchema: z.ZodType<SoftwareInfoType> = z.strictObject({
  /** The normalized identifier of the activity that triggered the event. */
...
export const SoftwareInfo = {
  parse: (data: unknown): SoftwareInfoType => SoftwareInfoSchema.parse(preprocess(data)),

  safeParse: (data: unknown): { success: true; data: SoftwareInfoType } | { success: false; error: z.ZodError } => {
    try {
      const preprocessed = preprocess(data);
      return SoftwareInfoSchema.safeParse(preprocessed);
    } catch (error) {
