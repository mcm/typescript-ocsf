 ...
 */
export interface UserInventoryType {
...
const UserInventorySchema: z.ZodType<UserInventoryType> = z.strictObject({
  /** The normalized identifier of the activity that triggered the event. */
...
export const UserInventory = {
  parse: (data: unknown): UserInventoryType => UserInventorySchema.parse(preprocess(data)),

  safeParse: (data: unknown): { success: true; data: UserInventoryType } | { success: false; error: z.ZodError } => {
    try {
      const preprocessed = preprocess(data);
      return UserInventorySchema.safeParse(preprocessed);
    } catch (error) {
