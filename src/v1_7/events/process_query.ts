 ...
 */
export interface ProcessQueryType {
...
const ProcessQuerySchema: z.ZodType<ProcessQueryType> = z.strictObject({
  /** The normalized identifier of the activity that triggered the event. */
...
export const ProcessQuery = {
  parse: (data: unknown): ProcessQueryType => ProcessQuerySchema.parse(preprocess(data)),

  safeParse: (data: unknown): { success: true; data: ProcessQueryType } | { success: false; error: z.ZodError } => {
    try {
      const preprocessed = preprocess(data);
      return ProcessQuerySchema.safeParse(preprocessed);
    } catch (error) {
