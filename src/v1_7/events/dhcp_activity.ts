...
const DhcpActivitySchema: z.ZodType<DhcpActivityType> = z.strictObject({
  /** The normalized identifier of the activity that triggered the event. */
...
export const DhcpActivity = {
  parse: (data: unknown): DhcpActivityType => DhcpActivitySchema.parse(preprocess(data)),

  safeParse: (data: unknown): { success: true; data: DhcpActivityType } | { success: false; error: z.ZodError } => {
    try {
      const preprocessed = preprocess(data);
      return DhcpActivitySchema.safeParse(preprocessed);
    } catch (error) {
